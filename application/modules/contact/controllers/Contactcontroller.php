	...
	class Contactcontroller extends MX_Controller
	{
		...
		public function exportToCsv($value='')
		{
			try {
				...
				fputcsv($f, ['Id','Brand Name','Modal Name','Service','Full Name','Mobile Number','Message','IP Address','Created Date']);
				$rows = $this->db->get('cr_contact_us')->result();
				foreach ($rows as $key => $value) {
					$data = [$key + 1, $value->brand_name,$value->modal_name,$value->service,$value->full_name, $value->mobile, $value->message, $value->ip_address,date('Y-M-d', strtotime($value->created_at))];
					fputcsv($f, $data);
				}
